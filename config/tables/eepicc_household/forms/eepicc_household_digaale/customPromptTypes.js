...
function(promptTypes, $, _, opendatakit, database) {
    ...
    configureRenderContext: function(ctxt) {
		...
		console.log("doing configureRenderContext 1");
        ...
        that.getlinkedModel($.extend({},ctxt,{success:function(linkedModel) {
			...
			console.log("doing configureRenderContext 2");
		...
		console.log("doing configureRenderContext 3");
        ...
        odkData.arbitraryQuery(dbTableName, ss.stmt, ss.bind, null, null,
            function(reqData) {
                ...
                for (var rowCntr = 0; rowCntr < reqData.getCount(); rowCntr++) {
                    ...
                    instanceList.push({
						...
						participant_id: reqData.getData(rowCntr, 'participant_id'),
                        instance_id: reqData.getData(rowCntr, '_id'),
						participant_selected: reqData.getData(rowCntr, 'participant_selected'),
                        savepoint_timestamp: ts,
			...
			console.log("doing configureRenderContext 4");
		}
		
		console.log("doing configureRenderContext 5");
            ...
            database2.get_linked_instances2($.extend({},ctxt,{success:function(instanceList) {
				...
				filteredInstanceList = _.filter(baklist, function(instance) {
					...
					//assume 450 households, no refusal, and hargeisa household member distribution
					var sp = 0;
					if(instance.age_years == 0) {sp = 1} else
					if(instance.age_years == 1) {sp = 1} else
					if(instance.age_years >= 2 && instance.age_years <= 4) {sp = 0.714} else
					if(instance.age_years >= 5 && instance.age_years <= 14) {sp = 0.302} else
					if(instance.age_years >= 15 && instance.age_years <= 29) {sp = 0.604} else
					if(instance.age_years >= 30 && instance.age_years <= 49) {sp = 0.821} else
					if(instance.age_years >= 50) {sp = 1} else
					sp = 0;
					
					// Get participant ID
					var participant_id = instance.participant_id;
					var participant_selected = instance.participant_selected;
					// Create cyrb128 state using participant ID
					var seed = cyrb128((participant_id).toString());
					// Four 32-bit component hashes provide the seed for sfc32.
					var rand = sfc32(seed[0], seed[1], seed[2], seed[3]);
					instance.rand = rand();			
					
					//always true if selected previously (based on different age)
					return (instance.rand <= sp || participant_selected == 1);
                });
                ...
                for (var i = 0; i < instanceList.length ; i++){
                    ...
                    if (instanceList[i].form_id != that.getLinkedFormId() && instanceList[i].form_id != "eepicc_vaccine_registration") {
                        instanceList[i].savepoint_type = opendatakit.savepoint_type_incomplete;
						console.log("zlog: instanceList["+i+"].form_id ("+instanceList[i].form_id+") != that.getLinkedFormId() ("+that.getLinkedFormId()+")");
                    }

                    if (instanceList[i].savepoint_type == "COMPLETE"){
                        instanceList[i].data_complete = 1;
						instanceList[i].icon_class = "glyphicon-ok";
                    }
                    else{
						instanceList[i].data_complete = 0;
						all_complete = false;
				...
				*/
				function assign(valueName, value) {
					'use strict';
				...
				if(all_complete){
					assign("swab_complete", 1);
					console.log("swab complete");
				} else {
					assign("swab_complete", 0);	
					console.log("swab not complete");
				}
    ...
    configureRenderContext: function(ctxt) {
		...
		console.log("doing configureRenderContext");
        ...
        odkData.arbitraryQuery(dbTableName, ss.stmt, ss.bind, null, null,
            ...
            database2.get_linked_instances2($.extend({},ctxt,{success:function(instanceList) {
                ...
                for (var i = 0; i < instanceList.length ; i++){
                    // sets the savepoint_type to incomplete if the formId doesn't match the current form
                    //if (instanceList[i].form_id != that.getLinkedFormId() && instanceList[i].form_id != "eepicc_individual_survey_digaale") {
				...
				*/
				...
				if(all_complete){
					assign("vacreg_complete", 1);
				} else {
					assign("vacreg_complete", 0);	
				}
